...
private[repository] trait BankInfoTable extends BankTable { this: DBComponent =>
  ...
  private[BankInfoTable] class BankInfoTable(tag: Tag) extends Table[BankInfo](tag, "bankinfo") {
    ...
    def * : ProvenShape[BankInfo] = (owner, branches, bankId, id.?) <> (BankInfo.tupled, BankInfo.unapply)
